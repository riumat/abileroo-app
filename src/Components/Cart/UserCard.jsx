...
const UserCard = ({ setDeliverDate, isError, setAddress }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-3 px-3 py-2 text-[13px] w-full border border-slate-400  rounded-lg ${isError?.user && "border-red-600 placeholder:text-red-400"}`}>
      ...
      <div className={`flex flex-col gap-3 px-3 py-2 text-[13px] w-full border border-slate-400  rounded-lg ${isError?.date && "border-red-600 placeholder:text-red-400"}`}>
        <p className='text-[15px] text-center'>Day</p>
        <input
          type="datetime-local"
          onChange={(e) => setDeliverDate(e.target.value)}
          className={`p-1 border-b dark:border-slate-700 bg-transparent dark:text-white text-[14px] ${isError?.date && " border-red-200"}`}
        />
      </div>
